...
source [file join $script_dir create_e310v2_sources.tcl]

set ::origin_dir_loc [file join $proj_dir bd]
set bd_status [source [file join $root_dir bd e310v2_ps_bd.tcl]]
if {$bd_status ne "" && $bd_status != 0} {
    error "E310V2 block-design creation failed with status $bd_status"
}
set bd_files [get_files -quiet -all "*e310v2_ps_bd.bd"]
if {[llength $bd_files] != 1} {
    error "expected one e310v2_ps_bd.bd, found [llength $bd_files]: $bd_files"
}
generate_target all $bd_files
make_wrapper -files $bd_files -top -import
set standalone_ips [list]
foreach rel $e310v2_source_rel_files {
...
puts "E310V2 project created: [file join $proj_dir ${proj_name}.xpr]"
close_project
