 ...
 **********************************************************************/
void e200_impl::check_tick_rate_with_current_streamers(double rate)
{
...
size_t e200_impl::max_chan_count(const std::string& direction /* = "" */)
{
    size_t max_count = 0;
...
void e200_impl::check_streamer_args(const uhd::stream_args_t& args,
                                    double tick_rate,
...
void e200_impl::set_auto_tick_rate(
...
void e200_impl::update_tick_rate(const double new_tick_rate)
{
...
void e200_impl::update_rx_dsp_tick_rate(
...
void e200_impl::update_tx_dsp_tick_rate(
...
double e200_impl::coerce_rx_samp_rate(
...
double e200_impl::coerce_tx_samp_rate(
 ...
 * frontend selection
 **********************************************************************/
uhd::usrp::subdev_spec_t e200_impl::coerce_subdev_spec(
        const uhd::usrp::subdev_spec_t& spec_)
{
    ...
    if (spec.size() and (_product == B200_ or _product == B200MINI or _product == B205MINI)
...
void e200_impl::update_subdev_spec(
 ...
 * Async Data
 **********************************************************************/
bool e200_impl::recv_async_msg(async_metadata_t& async_metadata, double timeout)
{
 ...
 * there.
 */
boost::optional<uhd::msg_task::msg_type_t> e200_impl::handle_async_task(
        uhd::transport::zero_copy_if::sptr xport, boost::shared_ptr<AsyncTaskData> data)
{
    ...
    switch (sid) {
        ...
        case E200_RESP0_MSG_SID:
        case E200_RESP1_MSG_SID:
        case E200_LOCAL_RESP_SID: {
            radio_ctrl_core_3000::sptr ctrl;
            if (sid == E200_RESP0_MSG_SID)
                ctrl = data->radio_ctrl[0].lock();
            if (sid == E200_RESP1_MSG_SID)
                ctrl = data->radio_ctrl[1].lock();
            if (sid == E200_LOCAL_RESP_SID)
        ...
        case E200_RX_GPS_UART_SID: {
            data->gpsdo_uart->handle_uart_packet(buff);
            break;
        }

            // or maybe the packet is a TX async message
        case E200_TX_MSG0_SID:
        case E200_TX_MSG1_SID: {
            const size_t i = (sid == E200_TX_MSG0_SID) ? 0 : 1;
...
rx_streamer::sptr e200_impl::get_rx_stream(const uhd::stream_args_t& args_)
{
    ...
    for (size_t stream_i = 0; stream_i < args.channels.size(); stream_i++) {
        ...
        const uint32_t sid = radio_index ? E200_RX_DATA1_SID : E200_RX_DATA0_SID;
        ...
        my_streamer->set_overflow_handler(
                stream_i, boost::bind(&e200_impl::handle_overflow, this, radio_index));
...
void e200_impl::handle_overflow(const size_t radio_index)
{
    boost::shared_ptr<sph::recv_packet_streamer> my_streamer =
            boost::dynamic_pointer_cast<sph::recv_packet_streamer>(
    ...
    if (my_streamer->get_num_channels() == 2) // MIMO time
    {
        ...
        // flush demux
        _demux->realloc_sid(E200_RX_DATA0_SID);
        _demux->realloc_sid(E200_RX_DATA1_SID);
...
tx_streamer::sptr e200_impl::get_tx_stream(const uhd::stream_args_t& args_)
{
    ...
    for (size_t stream_i = 0; stream_i < args.channels.size(); stream_i++) {
        ...
        my_streamer->set_xport_chan_sid(
                stream_i, true, radio_index ? E200_TX_DATA1_SID : E200_TX_DATA0_SID);
...
/* Microphase for e310 */
size_t e200_impl::_get_tx_flow_control_window(size_t payload_size, size_t hw_buff_size)
{
...
void e200_impl::_handle_tx_async_msgs(boost::shared_ptr<tx_fc_cache_t> fc_cache,
...
uhd::transport::managed_send_buffer::sptr e200_impl::_get_tx_buff_with_flowctrl(
        ...
        boost::shared_ptr<e200_impl::tx_fc_cache_t> fc_cache,
        uhd::transport::zero_copy_if::sptr xport,
