 ...
 **********************************************************************/
class e200_iface_impl : public e200_iface
{
public:
    e200_iface_impl(usb_control::sptr usb_ctrl) : _usb_ctrl(usb_ctrl)
    {
        // NOP
    }
    ...
    void load_firmware(const std::string filestring, UHD_UNUSED(bool force) = false)
    {
        ...
        try {
            file_reader.read(boost::bind(&e200_iface_impl::fx3_control_write,
                this,
                FX3_FIRMWARE_LOAD,
                _1,
                _2,
...
std::string e200_iface::fx3_state_string(uint8_t state)
{
    switch (state) {
 ...
 **********************************************************************/
e200_iface::sptr e200_iface::make(usb_control::sptr usb_ctrl)
{
    return sptr(new e200_iface_impl(usb_ctrl));
}
