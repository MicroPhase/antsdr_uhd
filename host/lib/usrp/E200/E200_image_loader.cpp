...
static e200_iface::sptr get_e200_iface(
    ...
    bool user_specified)
{
    std::vector<usb_device_handle::sptr> dev_handles =
        get_e200_device_handles(image_loader_args.args);
    std::vector<usb_device_handle::sptr> applicable_dev_handles;
    e200_iface::sptr iface;
    ...
    if (dev_handles.size() > 0) {
        for (usb_device_handle::sptr dev_handle : dev_handles) {
            if (dev_handle->firmware_loaded()) {
                iface  = e200_iface::make(usb_control::make(dev_handle, 0));
                eeprom = e200_impl::get_mb_eeprom(iface);
                if (user_specified) {
        ...
        if (applicable_dev_handles.size() == 1) {
            ...
            for (usb_device_handle::sptr dev_handle : applicable_dev_handles) {
                eeprom = e200_impl::get_mb_eeprom(
                    e200_iface::make(usb_control::make(dev_handle, 0)));
                err_msg += str(
                    boost::format(" * %s (serial=%s)\n")
                    % E2XX_STR_NAMES.get(get_e200_product(dev_handle, mb_eeprom), "B2XX")
...
static bool e200_image_loader(const image_loader::image_loader_args_t& image_loader_args)
{
    ...
    usb_device_handle::sptr handle;
    e200_iface::sptr iface =
        get_e200_iface(image_loader_args, mb_eeprom, handle, user_specified);
    if (!iface)
    ...
    if (image_loader_args.fpga_path == "") {
        ...
        if (not E2XX_PRODUCT_ID.has_key(boost::lexical_cast<uint16_t>(product))) {
            if (user_specified) {
            ...
            fpga_path = find_image_path(
                E2XX_FPGA_FILE_NAME.get(get_e200_product(handle, mb_eeprom)));
        }
    } else
        fpga_path = image_loader_args.fpga_path;

    std::cout << boost::format("Unit: USRP %s (%s)")
                     % E2XX_STR_NAMES.get(get_e200_product(handle, mb_eeprom), "B2XX")
                     % mb_eeprom.get("serial")
              << std::endl;
...
UHD_STATIC_BLOCK(register_e200_image_loader)
{
    ...
    image_loader::register_image_loader("e200", e200_image_loader, recovery_instructions);
}
