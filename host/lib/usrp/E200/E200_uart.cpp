...
struct e200_uart_impl : e200_uart
{
    e200_uart_impl(zero_copy_if::sptr xport, const uint32_t sid)
        : _xport(xport)
        , _sid(sid)
        , _count(0)
        , _baud_div(std::floor(E200_BUS_CLOCK_RATE / 115200 + 0.5))
        , _line_queue(4096)
    {
        /*NOP*/
    }
...
e200_uart::sptr e200_uart::make(zero_copy_if::sptr xport, const uint32_t sid)
{
    return e200_uart::sptr(new e200_uart_impl(xport, sid));
}
